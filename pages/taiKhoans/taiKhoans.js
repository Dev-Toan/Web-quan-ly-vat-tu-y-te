$(document).ready(function () {
        ...
        render: function () {
          ...
          <button 
                    ...
                    <button 
                                ...
                                data-bs-toggle="modal"
                                data-bs-target="#deleteTaiKhoanModal"
    ...
    switch (buttonClass) {
      ...
      case "btn btn-outline-danger btn-delete":
        $("#delete-form-btn").on("click", function () {
          $.ajax({
            url: `http://localhost:3000/nhanSus/${data_PB.id}`,
            type: "DELETE",
          })
            .done(function (ketqua) {
              console.log(ketqua);
  ...
  $('#btnExportExcel').on('click',function(){
    ...
    var data = taiKhoansTable.rows().data().toArray();

    //chuyen du lieu thanh array 2 chieu de xuat excel
    var exportData= [
        ["Tên tài khoản", "Mật khẩu", "Quyền", "Tên nhân sự", "Giới tính", "Ngày sinh", "SĐT", "Email", "Phòng ban","Chức vụ"]// tiêu đề cột
    ];
    ...
    data.forEach(function(row){
        exportData.push([
            ...
            row.matKhau,
            row.role,
            // row.role === 'user' ? 'Người dùng' : 'Quản trị viên',
            row.tenNhanSu,
            // row.phongBan ? row.phongBan.tenPhongBan : '',
            row.gioiTinh,
            row.ngaySinh,
            row.soDienThoai,
            row.email,
